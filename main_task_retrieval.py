...
def get_args(description='CLIP4Clip on Retrieval Task'):
    ...
    parser.add_argument("--init_model", default="./ckpts/ckpt_msvd_retrieval_looseType_tightTransf/pytorch_model.bin.3", type=str, required=False, help="Initial model.")
...
def init_model(args, device, n_gpu, local_rank):
    ...
    if args.init_model and args.do_test:
        print("-=-=-=-=-=-=-=ckpt load-=-=-=-=-=-=-=-=-")
...
def eval_epoch(args, model, test_dataloader, device, n_gpu):
    ...
    visualization_result(sim_matrix.reshape(-1,sim_matrix.shape[-1]))
    ...
    R1 = tv_metrics['R1']#(vt_metrics['MeanR']+tv_metrics['MeanR'])/2#
    return R1

def visualization_result(one_sims):
        
    rs=np.argsort(one_sims,axis=1)+1
    ...
    f=open("Result_text_img.txt","a+")
    f.write(result)

    rs=np.sort(one_sims,axis=1)
    ...
    f=open("Result_text_img_weight.txt","a+")
    f.write(result)
